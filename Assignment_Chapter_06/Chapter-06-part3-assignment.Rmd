...
m1 <- map(
  ...
  data=d1 , start=list(a=a.start,b1=0,log.sigma=sigma.start) )
#second order model
m2 <- map(
  alist(
   height ~ dnorm( mu , exp(log.sigma) ) ,
  ...
  data=d1 , start=list(a=a.start,b1=0,b2=0,log.sigma=sigma.start) )
#third order model
m3 <- map(
  alist(
   height ~ dnorm( mu , exp(log.sigma) ) ,
  ...
  data=d1 , start=list(a=a.start,b1=0,b2=0,b3=0,log.sigma=sigma.start) )
#fourth order model
m4 <- map(
  alist(
   height ~ dnorm( mu , exp(log.sigma) ) ,
  ...
  data=d1 , start=list(a=a.start,b1=0,b2=0,b3=0,b4=0,log.sigma=sigma.start) )
#fifth order model
m5 <- map(
  alist(
   height ~ dnorm( mu , exp(log.sigma) ) ,
  ...
  data=d1 , start=list(a=a.start,b1=0,b2=0,b3=0,b4=0,b5=0,log.sigma=sigma.start) )
#sixth order model
m6 <- map(
  alist(
   height ~ dnorm( mu , exp(log.sigma) ) ,
  ...
  data=d1 , start=list(a=a.start,b1=0,b2=0,b3=0,b4=0,b5=0,b6=0,log.sigma=sigma.start) )
...
n_samples <- 1000
ll.m1 <- sapply( 1:n_samples ,
function(s) {
mu <- post.m1$a[s] + post.m1$b1[s]*d2$age
dnorm( d2$height , mu , post.m1$log.sigma[s] , log=TRUE )
} )

#compute Bayesian deviance lppd
n_cases <- nrow(d2) 
lppd.m1 <- sapply( 1:n_cases , function(i) log_sum_exp(ll.m1[i,]) - log(n_samples) )
...
dnorm( d2$height , mu , post.m2$log.sigma[s] , log=TRUE )
} )
n_cases <- nrow(d2) 
...
dnorm( d2$height , mu , post.m3$log.sigma[s] , log=TRUE )
} )
n_cases <- nrow(d2) 
...
dnorm( d2$height , mu , post.m4$log.sigma[s] , log=TRUE )
} )
n_cases <- nrow(d2) 
...
dnorm( d2$height , mu , post.m5$log.sigma[s] , log=TRUE )
} )
n_cases <- nrow(d2) 
...
dnorm( d2$height , mu , post.m6$log.sigma[s] , log=TRUE )
} )
n_cases <- nrow(d2) 
...
Fit model to d1.
Report MAP estimates, plot predictions.
Compute out-of-sample deviance using d2
How does this compare to best WAIC from earlier?